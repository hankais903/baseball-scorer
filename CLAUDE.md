...
- **比賽中可以臨時新增板凳球員**（v2.33）：比賽畫面進不去名單頁，板凳沒人時整個換人流程會卡死，
  所以「選擇球員」視窗下面放了「＋ 臨時新增〈隊名〉的球員」（背號＋姓名 → 加入並直接換上）。
  `addBenchPlayer()` 會找一個還空著的板凳格（跳過先發九棒與投手那一格）填名字。
  **一定要記成紙條**（`{ t:'addp', team, id, name, jersey }`，重播時走 `applyAddPlayer`）：
  重播重建的是開賽當下的名單，不記的話這個人會變回沒名字的空格、整場對不上。已有測試釘住。
...
- **建立比賽是三步**（`#page-game`）：比賽資訊（日期／時間／球場／天氣／對手隊名／我方先攻或後攻）
  ...
  **板凳沒填名字的人一定要補一個名字（簡稱＋背號）**：`name === ''` 在這個專案代表「沒這個人」，
  留空白的話板凳整排被當成空的，比賽中代打／代跑／換投一個人都選不到（踩過一次，已有測試釘住）。
