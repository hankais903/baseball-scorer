...
- **換人的種類要分清楚**（`processSubstitution`）：只有「這一刻正要打擊的人被換掉」才是代打；
  守備中途換人（換下的人不是現在的打者）寫**守備替補**，投手寫更換投手，壘上跑者寫代跑。
  以前 `spotIndex !== -1` 就一律寫成「代打」，換個守備員也會記成代打（踩過一次，已有測試釘住）。
...
## 打席結果的簡稱（戰況表與打者卡的本場結果，`situationLabel`）
- **中文數字／方位＝球打去哪個方向，阿拉伯數字＝上到第幾壘**（使用者指定，v2.36）：
  `左2`＝左外野方向的二壘安打、`二安`＝二壘方向的一壘安打、`右3`＝右外野三壘安打、`中H`＝中外野全壘打。
  沒有方向時退回 `安`／`2`／`3`／`HR`。內野安打是 `游內安`，場地二壘打是 `左場2`。
- 出局類照舊：`游滾`、`中飛`、`界飛`、`犧飛`、`犧短`、`三殺`、`左平雙殺`、`游失`、`野選`。
- **打者卡的「本場」與戰況表走同一個函式**（`situationLabel`），不要各寫一套。
